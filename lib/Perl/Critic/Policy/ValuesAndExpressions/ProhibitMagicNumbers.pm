...
sub _postponed_supported_parameters {
    return (
        {
            name           => 'allowed_values',
...
sub new {
    ...
    ## temporary back-pedaling:
    ## delete these six lines when we get config support
    $self->_parse_allowed_values('allowed_values', $config{allowed_values});
    my $allowed_types = $config{allowed_types};
    if (not defined $allowed_types) {
        $allowed_types = 'Float';
    }
    $self->{_allowed_types} = { hashify( words_from_string($allowed_types) ) };
